"""관리자 정책을 바꾸며 전체 PII 파이프라인을 점검하는 Streamlit 화면.

이 파일은 화면 구성과 사용자 입력을 담당한다. 개인정보 탐지 규칙을 직접 구현하지
않고 ``src/pii_masking``의 실제 엔진을 호출한다.

코드 검토 순서
1. ``load_and_process()``에서 OCR JSON 읽기와 전처리 cache를 확인한다.
2. ``load_aegis_runtime()``에서 AEGIS 모델 session cache를 확인한다.
3. ``main()``의 첫 번째 sidebar에서 파일·이미지·정책·모델 경로를 선택한다.
4. 두 번째 sidebar에서 민감도, 탐지기 실행·병합 방식과 Gate 값을 조정한다.
5. ``MaskingPolicyConfig``와 ``SearchViewSettings``를 조립한다.
6. ``detect_document_pii()``로 실제 엔진을 한 번 실행한다.
7. OCR → 전처리 → 탐지 → span·bbox → 마스킹 탭에서 중간 결과를 표시한다.

표와 좌표 이미지처럼 화면 표시만을 위한 변환은 ``dashboard_support.py``에 있다.
배치 실행 흐름은 프로젝트 루트의 ``run_full_pipeline.py``에서 확인한다.
"""
...
# ``streamlit run apps/streamlit_app.py``처럼 직접 실행해도 프로젝트 모듈을 찾도록
...
EXECUTION_MODE_NAMES = {
    ...
    "category_routing": "개인정보 유형별 탐지기 분담",
}
MERGE_MODE_NAMES = {
    "any": "한 탐지기만 찾아도 유지",
...
def load_and_process(
    ...
    """OCR JSON을 읽고 모든 문서를 전처리한 결과를 Streamlit cache에 저장한다.

    OCR JSON 수정 시각과 PIPELINE_CACHE_VERSION이 바뀌면 cache를 새로 만든다.
    개인정보 탐지는 정책에 따라 즉시 달라져야 하므로 여기서 실행하지 않는다.
    """
    del modified_ns
...
def load_aegis_runtime(model_dir: str) -> AegisOnnxNer:
    """무거운 ONNX session을 화면 재실행마다 다시 만들지 않도록 보관한다."""
...
def _reset_policy_widgets() -> None:
    ...
    for key in (
        ...
        "policy_custom_person_fields",
        "policy_custom_address_fields",
...
def main() -> None:
    """관리자 설정을 받아 현재 문서의 단계별 결과를 화면에 표시한다."""
    # 1. 페이지 기본 정보와 입력 파일 경로를 구성한다.
    st.set_page_config(
    ...
    # 2. policy.json과 OCR 결과 목록을 읽는다. 여기의 saved_policy_config는
    # sidebar 위젯의 초기값이며, 아직 현재 화면 설정을 적용한 정책은 아니다.
    try:
    ...
    # 선택한 OCR JSON을 OcrDocument와 PreprocessedDocument 목록으로 바꾼다.
    # Ctrl+클릭: load_and_process → ocr_parser → preprocessing 순서로 이어진다.
    try:
    ...
    # 3. 관리자가 실험할 탐지·마스킹 정책을 sidebar에서 입력받는다.
    with st.sidebar:
        ...
        with st.expander("관리자 추가 필드명"):
            custom_person_fields_text = st.text_area(
                "이름 필드명",
                value=", ".join(
                    saved_policy_config.custom_person_field_labels
                ),
                placeholder="예: 학생명, 수강생, 담임교사",
                help="쉼표 또는 줄바꿈으로 구분합니다.",
                key="policy_custom_person_fields",
            )
            custom_address_fields_text = st.text_area(
                "주소 필드명",
                value=", ".join(
                    saved_policy_config.custom_address_field_labels
                ),
                placeholder="예: 배송지, 설치장소, 근무지",
                help="쉼표 또는 줄바꿈으로 구분합니다.",
                key="policy_custom_address_fields",
            )
    ...
    # 4. 화면 위젯 값을 엔진이 사용하는 정책 객체로 조립한다.
    # 화면 설정을 곧바로 전역변수에 쓰지 않으므로 같은 형식을 API와 C++에서도
    # 전달할 수 있다.
    custom_person_fields = _parse_field_labels(custom_person_fields_text)
    custom_address_fields = _parse_field_labels(custom_address_fields_text)
    aegis_scope = (
        "semantic"
    ...
    policy_config = MaskingPolicyConfig(
        ...
        custom_person_field_labels=custom_person_fields,
        custom_address_field_labels=custom_address_fields,
    )
    ...
    # SearchView 생성에 필요한 전처리 관련 옵션만 분리한다.
    search_settings = SearchViewSettings(
        ...
        custom_person_field_labels=policy_config.custom_person_field_labels,
        custom_address_field_labels=policy_config.custom_address_field_labels,
    )
    ...
    # 반환값에는 ko-pii 결과, AEGIS 결과, 정책 적용 후 최종 결과가 모두 들어 있다.
    try:
    ...
    # 6. 아래 값은 진단 화면 표시용이다. 실제 최종 span을 다시 계산하지 않는다.
    # SearchView와 원시 예측 trace를 별도로 만들어 각 단계에서 제외된 이유를 보여 준다.
    ...
    # 7. 동일한 처리 결과를 단계별 탭에 나누어 표시한다.
    # 각 탭은 엔진을 새로 구현하지 않고 위에서 계산한 객체를 표와 이미지로 보여 준다.
    (
        ocr_tab,
        preprocessing_tab,
        detection_tab,
